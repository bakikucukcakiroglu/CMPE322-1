	...
	*/
	...
	while(1){
		...
		if(argt[i]=='\0') { 				// That is the end of string. We have to stop reading. 
			...
			part_c_1_arg.path[i]='\0';		// We add a null terminator to the end of the string to provent anormal behaviours. 
			break;							// We stop when we reached the end of the string.
		}

		part_c_1_arg.path[i]=argt[i]; // ....
		i++;

	}

	
	result_1 = part_c_1(&part_c_1_arg, clnt);	// The result that returned by server process. It will later be processed.
