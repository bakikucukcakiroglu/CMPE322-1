 ...
 * operations are done there and output is written appropriate file which is given as command line arguments. In addition to all 
 * these operations, server of part C sends input and output of the blackbox program to the logger program. This program is a server,
 * takes necessary inputs and putputs via sockets and ports. It later writes all of them to a output file accordingly.
 * 
 */
...
/**
 * These arguments from main of the server. We cannot reach them directly so we extern them. With this way all parameters 
 * are described as global and can be reached from the files that extern them.
 */ 
 ...
 */
char **
part_c_1_svc(inputs *argp, struct svc_req *rqstp)
{
  /**
   * 
  ...
  if(pid > 0) {
    ...
    /**
       ...
       * Client takes necessary actions to write the into apropriate files. This program sends inpıt and output to logger program via sockets. 
       * 
       */
        ...
        int sock = 0, valread;        //Socket number
        struct sockaddr_in serv_addr; // Address of the server. 
       
        /*
        * Creates a socket which will ensure the communication between server and logger.
        */
        ...
        serv_addr.sin_family = AF_INET; // Sets IP adress type of the server.
        serv_addr.sin_port = htons(pP); // Sets port number of the server. 

        /*
        * It checks and arrange the IP numbers of the server. If it is invalid, it promots to the screen.
        */
        inet_pton(AF_INET, hostName, &serv_addr.sin_addr); 
       

        /*
        * It tries to connect logger via sockets. 
        */
        ...
        */
        ...
        if((size=read(c2p[0], buffer, sizeof(buffer)))){
          ...
          *
          */
          int strSize=strlen(p_str);  //Size of the input string.
          p_str[strSize]='\0';        // We add null terminator to the end of buffer to prevent it behave anormally.
          strcat(p_str, buffer);      // Concatanate input with output that is come from blackbox.

          /**
          * Sends input and output to server(logger) via sockets. They are wrote a file in logger program.
          */
          send(sock , p_str , strlen(buffer)+strlen(p_str) , 0 ); 
        ...
        */
        ...
        if((size2=read(errc2p[0], buffer, sizeof(buffer)))){
          ...
          * Sends input and output to server(logger) via sockets. They are wrote a file in logger program.
          */
          send(sock , p_str , strlen(p_str)+1 , 0 );
